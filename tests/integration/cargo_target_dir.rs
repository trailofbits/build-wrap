...
const DIR: &str = "fixtures/cargo_target_dir";
...
fn cargo_target_dir() {
    ...
    // `target`. So building the wrapper package in `fixtures/cargo_target_dir` would fail because
    // it contains a `.cargo/config.toml` that sets the target directory to `target-custom`.

    // smoelius: The build script in `fixtures/cargo_target_dir` prints the path of the current
    ...
    // Note that the `fixtures/cargo_target_dir` directory must be cleaned for it to be rebuilt.
...
fn test_build(mut command: Command) {
    ...
    assert!(stderr.lines().any(|line| line.starts_with(&format!(
        "warning: cargo_target_dir@0.1.0: {}/",
