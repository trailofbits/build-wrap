...
fn config_toml_packages(config_contents: &str) {
    ...
    // Each iteration needs a fresh temp_package because the config check happens at link time. Cargo
    // caches the linked build script, so a second `cargo build` in the same directory would reuse
    // the artifact from the first iteration.
